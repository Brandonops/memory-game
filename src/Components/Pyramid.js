...
class Cylinder extends Component {
  ...
  addModels() {
    // -----Step 1--------
    const geometry = new THREE.CylinderGeometry(0, 10, 22, 3, 10, false, 0, 6.3);
    ...
    new THREE.TextureLoader().load(
      ...
      texture => {
        //Update Texture
        this.pyramid.material.map = texture;
  ...
  animate = () => {
    // -----Step 3--------
    //Rotate Models
    if (this.pyramid) this.pyramid.rotation.y += -0.005;
    if (this.pyramid) this.pyramid.rotation.x += -0.005;
    if (this.pyramid) this.pyramid.rotation.z += -0.005;
